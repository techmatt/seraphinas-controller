...
- **Family (entirely made-up, not the real family):** one dad and **Hazel**, Seraphina's little sister. They wander room-to-room on a simple schedule, always *visibly doing something* (chopping wood, drawing, stacking blocks) — an NPC's "life" is their animation loop. **Dad is now placed** (v0, 2026-08-15): the pack's premade Farmer_Bob (the paper doll is child-body-only — adult NPCs use the premade sheets), static beside his shed, walk-through, two idle lines, and the bunny-rescue giver, relocating to the den for its duration. His dusk call-in stays a disembodied voice from the front door — he is calling her in from indoors. Schedules and activity loops remain future.
...
**Offer model** (Matt, 2026-08-13). Every quest not completed today shows its giver's thought bubble; ONE active quest is absolute; completing one immediately restores the others' offers; a completed quest stays blocked until sleep. Two-quest days are the shipped shape. **One quest per NPC** (Matt, 2026-08-15) until Julia's longer playthrough: a giver with two quests would serialize them (`offerFrom` gives the giver's first quest not done today) — the wrong day shape. The bunny rescue therefore moved from Hazel to Dad; mornings show three bubbles (Sneak, Dad, Hazel), one quest each.
...
**Quest #3 — Storytime, now canon** (Matt, 2026-08-14; shipped 2026-08-15). Hazel gives it — her only quest — and relocates to the living-room reading nook for its duration. Phases: get the book (gather 1; it spawns at the house bookshelf) → come read with Hazel (travel) → read the story (the `book` goal kind: A at the nook opens the takeover reader; the phase completes when the last page is read) → hug-flavored completion + coin. Reader grammar: each page auto-reads in Seraphina's voice with per-word highlight; green during the read is IGNORED, green after it turns the page (juicy flourish), yellow re-reads the page, red closes friendly with the page HELD — reopening resumes, progress never regresses. Hazel delights in her own voice per page turn, bigger after the last. Book #1 is "Pip Goes to the Moon": Pip zooms to the moon, meets a baby dragon, gets sneezed with sparkles, best friends. A new book is a data addition (`content/books/`) plus genAI page art; undrawn pages show a friendly placeholder card, so a book is playable with zero art.
...
5. **Read a book to your little sister** — the reading flagship, SHIPPED as quest #3 (the canon block in Quests supersedes this sketch). Big two-page takeover book UI: picture left, ONE ACTION SENTENCE right (Matt, 2026-08-15: Julia reads at sentence level — single words bore her); audio reads with per-word highlighting; green turns pages; Hazel delights per page; ends in a hug + coin. Books are 4-page genAI-illustrated data sets — endless new content.
...
- Best genAI sinks: outfits, stickers, book pages, dragon variants, room decorations.
- Book page images: Matt genAI's them from the ChatGPT storybook template claude.ai keeps in memory (regenerated filled-in per book; sentences and scene notes authored together in planning). They live in the side-load at `stories/<book_id>/pageN.png` (UNDERSCORE ids), synced as an OPTIONAL category; `assets:sync` writes countable-dot placeholder PNGs for undrawn pages (never overwriting), and the reader's drawn card is the runtime fallback.
